...
impl ConcurrentDocids {
    ...
    pub fn next(&self) -> anyhow::Result<DocumentId> {
        if self.used.fetch_add(1, Ordering::Relaxed) > u32::MAX as u64 {
            anyhow::bail!("no more ID available")
