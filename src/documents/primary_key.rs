...
pub enum DocumentIdExtractionError {
    InvalidDocumentId(anyhow::Error),
    MissingDocumentId,
...
impl<'a> PrimaryKey<'a> {
    ...
    ) -> anyhow::Result<StdResult<String, DocumentIdExtractionError>> {
        match self {
...
pub fn validate_document_id_value(document_id: Value) -> anyhow::Result<String> {
    match document_id {
        Value::String(string) => match validate_document_id(&string) {
            Some(s) if s.len() == string.len() => Ok(string),
            Some(s) => Ok(s.to_string()),
            None => anyhow::bail!("Invalid document id `{string}`"),
        },
        Value::Number(number) if !number.is_f64() => Ok(number.to_string()),
        content => anyhow::bail!("Invalid document id `{content:?}`"),
    }
}
